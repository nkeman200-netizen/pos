                ...
                <div class="bg-white dark:bg-slate-800 p-6 lg:p-8 rounded-3xl shadow-sm border border-red-100 dark:border-red-500/20">
                    ...
                    <form wire:submit.prevent="save" class="space-y-4">
                        ...
                        <div>
                            ...
                            <select wire:model.live="productId" class="w-full p-2.5 bg-gray-50 dark:bg-slate-900 border border-gray-200 dark:border-slate-600 rounded-xl outline-none focus:ring-2 focus:ring-red-500 dark:text-white text-sm font-bold">
                                <option value="">-- Cari Obat --</option>
                                @foreach($products as $p)
                                    <option value="{{ $p->id }}">{{ $p->name }}</option>
                                @endforeach
                            </select>
                            @error('productId') <span class="text-xs font-bold text-red-500 mt-1">{{ $message }}</span> @enderror
                        </div>
